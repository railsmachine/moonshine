= Moonshine

Moonshine is Rails deployment and configuration management done right.

By leveraging capistrano and puppet, moonshine allows you have a working application server in 15 minutes, and be able to sanely manage it's configuration from the comfort of your version control of choice.

* {Step-by-step instructions for getting started}[https://github.com/railsmachine/moonshine/wiki/Tutorial]
* {Managing server configuration with ShadowPuppet}[https://github.com/railsmachine/moonshine/wiki/shadow-puppet-overview-and-examples]
* {Managing gems}[https://github.com/railsmachine/moonshine/wiki/Adding-Gems]
* {Managing moonshine.yml}[https://github.com/railsmachine/moonshine/wiki/Configuration]
* {Managing additional Apache vhosts}[https://github.com/railsmachine/moonshine/wiki/managing-additional-apache-vhosts]
* {Moonshine plugins}[https://github.com/railsmachine/moonshine/wiki/Moonshine-plugins]
* {Details about capistrano-integration}[https://github.com/railsmachine/moonshine/wiki/Moonshine-and-Capistrano]
* {Rails 3 notes}[https://github.com/railsmachine/moonshine/wiki/Rails-3]

== Requirements

* A server running Ubuntu 8.10 or 10.04 LTS (Want to see your favorite platform supported?  Fork Moonshine on GitHub!)
* A Rails 2.3 or Rails 3 app
* A user on this server that can:
  * Execute commands via sudo
  * Access your application's source code repository

== Running Tests

It's easy enough:

    $ gem install shadow_puppet isolate-scenarios
    $ rake spec

{isolate-scenarios}[http://github.com/technicalpickles/isolate-scenarios] is used to test against multiple versions of Rails. To run all scenarios at once:

   $ isolate-scenarios rake spec

== Getting Help

You can find more examples in the documentation[http://railsmachine.github.com/moonshine] and on the Wiki[http://wiki.github.com/railsmachine/moonshine].

For help or general discussion, visit the Moonshine newsgroup[http://groups.google.com/group/railsmachine-moonshine].
